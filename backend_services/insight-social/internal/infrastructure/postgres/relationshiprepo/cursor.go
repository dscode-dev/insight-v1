...
// Relationship cursor: base64url("<RFC3339Nano>|<bigint>").
// Same shape as signal/discussion-message cursors — the
// relationships table uses BIGSERIAL ids too.

func encodeRelCursor(ts time.Time, id int64) string {
	if ts.IsZero() {
		return ""
	}
...
func decodeRelCursor(cursor string) (time.Time, int64, error) {
	if cursor == "" {
