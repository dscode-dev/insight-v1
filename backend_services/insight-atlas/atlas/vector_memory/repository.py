...
class PgVectorMemoryRepository:
    ...
    async def upsert_many(self, embeddings: list[MemoryEmbedding]) -> int:
        # ON CONFLICT target is (source_match_id, embedding_version) —
        # migration 0018 widened the unique constraint from a bare
        # source_match_id so a v2 row (upsert_many_v2) can coexist for
        # the same match. Every MemoryEmbedding here still carries
        # EMBEDDING_VERSION (v1), so behavior is unchanged in practice.
        if not embeddings:
            return 0
        statement = text(
            """
            ...
            ON CONFLICT (source_match_id, embedding_version) DO UPDATE SET
    ...
    async def upsert_many_v2(self, embeddings: list[MemoryEmbeddingV2]) -> int:
        """Same shape as `upsert_many`, writing into the SEPARATE
        `embedding_v2` column (migration 0018) — `embedding` (v1) is
        never touched by this path. A match with an existing v1 row
        gets a SECOND row here (v1 and v2 are distinct embedding_version
        values, so the migration-0018 composite unique constraint
        `(source_match_id, embedding_version)` treats them as different
        keys) — never overwrites or merges into the v1 row."""
        if not embeddings:
            return 0
        statement = text(
            """
            INSERT INTO atlas.atlas_vector_memory (
                ...
                CAST(:trends AS jsonb), CAST(:signals AS jsonb),
                :market_available, :uncertainty, :embedding_version,
                CAST(:embedding_v2 AS vector), :created_at,
                :feature_schema_version, :signal_catalog_version,
                :behavior_catalog_version, :season, :market_type, :match_phase,
                CAST(:similarity_metadata AS jsonb)
            )
        ...
        async with self._sf() as session:
            await session.execute(
                statement,
                [_params_v2(embedding) for embedding in embeddings],
            )
            await session.commit()
        return len(embeddings)
...
def _params_v2(embedding: MemoryEmbeddingV2) -> dict:
    return {
        "vector_id": embedding.vector_id,
        "source_match_id": embedding.source_match_id,
        "competition": embedding.competition,
        ...
        "uncertainty": embedding.uncertainty,
        "embedding_version": embedding.embedding_version,
        "embedding_v2": _vector(embedding.embedding),
        "created_at": embedding.created_at,
        **compatibility_params(
            source="deterministic_vector_memory",
            market_type="match_odds" if embedding.market_available else None,
