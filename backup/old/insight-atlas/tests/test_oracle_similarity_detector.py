...
def _match(
    ...
    *,
    ...
    """Um vizinho, COM desfecho.

    O desfecho é obrigatório na prática desde que o portão passou a medir
    concordância de verdade. Antes ele media 1 - amplitude/média das
    distâncias e chamava aquilo de acordo entre vizinhos — um número que não
    precisa de desfecho porque não fala sobre desfecho.
    """
...
def test_emits_pattern_only_on_tighter_gate() -> None:
    ...
    trends = OracleSimilarityDetector().detect(
        _inputs(_result(matches, agreement=0.7))
    )
    types = {t.trend_type for t in trends}
    assert TrendType.historical_similarity in types
...
def test_no_pattern_when_neighborhood_is_thin() -> None:
    ...
    trends = OracleSimilarityDetector().detect(
        _inputs(_result(matches, agreement=0.9))
    )
    types = {t.trend_type for t in trends}
    assert TrendType.historical_similarity in types
...
def test_emits_nothing_quando_os_vizinhos_discordam_do_desfecho() -> None:
    """CONCORDÂNCIA AGORA É SOBRE DESFECHO, não sobre distância.

    Quatro vizinhos parecidíssimos entre si que terminaram de quatro jeitos
    diferentes não descrevem nada — e o portão antigo os deixaria passar,
    porque as distâncias eram uniformes. É o caso exatamente invertido do que
    este teste checava antes.
    """
    matches = [
        _match("m0", 0.95, outcome="HOME_WIN"),
        _match("m1", 0.94, outcome="AWAY_WIN"),
        _match("m2", 0.93, outcome="DRAW"),
        _match("m3", 0.92, outcome="AWAY_WIN"),
        _match("m4", 0.91, outcome="HOME_WIN"),
    ]
    # 2 de 5 no desfecho modal = 40%, abaixo da taxa base padrão (45%) mesmo
    # antes da margem.
    assert OracleSimilarityDetector().detect(_inputs(_result(matches))) == []


def test_emite_quando_a_vizinhanca_supera_a_taxa_base() -> None:
    matches = [_match(f"m{i}", 0.9) for i in range(4)]  # todos HOME_WIN
    trends = OracleSimilarityDetector().detect(_inputs(_result(matches)))
    assert any(t.trend_type == TrendType.historical_similarity for t in trends)


def test_a_taxa_base_da_competicao_manda_e_nao_um_numero_fixo() -> None:
    """A MESMA vizinhança, dois campeonatos, dois vereditos.

    60% de vitórias do mandante entre os vizinhos é notável onde a taxa base
    é 43,3% (Premier League) e é rigorosamente o normal onde ela é 48,4%
    (Brasileirão) — se o portão exige 5 pontos de folga, 60% passa nos dois,
    mas 50% passa só no primeiro. Um limiar fixo não sabe a diferença.
    """
    from atlas.similarity.contracts import SimilarityContext

    matches = [
        _match("m0", 0.95, outcome="HOME_WIN"),
        _match("m1", 0.94, outcome="HOME_WIN"),
        _match("m2", 0.93, outcome="AWAY_WIN"),
        _match("m3", 0.92, outcome="DRAW"),
    ]  # concordância = 50%

    def _com_base(taxa: float):
        from atlas.similarity.contracts import SimilarityDistribution

        base = _result(matches)
        ...
        return SimilarityContext(
            ...
            top_k=base.top_k,
            minimum_similarity=base.minimum_similarity,
            agreement=0.5,
            coverage=1.0,
            distribution=SimilarityDistribution(
                count=len(matches),
                best_similarity=max(sims),
                worst_similarity=min(sims),
                mean_similarity=sum(sims) / len(sims),
                min_distance=min(dists),
                max_distance=max(dists),
                mean_distance=sum(dists) / len(dists),
                distance_spread=max(dists) - min(dists),
            ),
            embedding_version=EV,
            metadata={"taxa_base": taxa},
        )

    detector = OracleSimilarityDetector()
    assert detector.detect(_inputs(_com_base(0.433))) != []   # 50% > 43,3% + 5%
    assert detector.detect(_inputs(_com_base(0.484))) == []   # 50% < 48,4% + 5%


def test_sem_desfecho_nos_vizinhos_o_portao_nao_deixa_passar() -> None:
    """Um portão que não consegue medir não deve deixar passar por não
    conseguir. É o caminho antigo de `atlas_vector_memory`, que não guardava
    o desfecho ao lado do vetor."""
    matches = [
        SimilarityMatch(
            vector_id=uuid4(),
            match_id=f"m{i}",
