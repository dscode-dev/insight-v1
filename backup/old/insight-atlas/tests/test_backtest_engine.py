...
def _match(mid: str, sim: float, outcome: str = "HOME_WIN") -> SimilarityMatch:
    # `outcome` é obrigatório na prática: o portão do detector passou a medir
    # concordância de desfecho, e um vizinho sem desfecho não tem como ser
    # julgado — o detector recusa em vez de deixar passar por não conseguir.
    return SimilarityMatch(
        ...
        competition="Serie A", outcome=outcome,
    )
...
def _context(n: int = 4, agreement: float = 0.85) -> SimilarityContext:
    ...
    # `agreement` agora governa os DESFECHOS, não o leque de distâncias: é o
    # que o portão de fato mede. Alto = todos terminaram igual; baixo = cada
    # um terminou de um jeito.
    _DESFECHOS = ("HOME_WIN", "AWAY_WIN", "DRAW")
    matches = (
        [_match(f"m{i}", 0.92 - 0.01 * i) for i in range(n)]
        if agreement >= 0.4
        else [
            _match(f"m{i}", 0.92 - 0.01 * i, _DESFECHOS[i % len(_DESFECHOS)])
            for i in range(n)
        ]
    )
    ...
    return SimilarityContext(
        ...
        confidence=SimilarityConfidence(
            ...
            distance_spread=0.03, distance_uniformity=agreement,
        ),
