...
class SimilarityMatch(BaseModel):
    ...
    market_type: str | None = None
    match_phase: str | None = None
    #: Como esta partida vizinha terminou: HOME_WIN, DRAW, AWAY_WIN.
    #:
    #: SEM ISTO NÃO HÁ COMO MEDIR CONCORDÂNCIA, e é por isso que o portão do
    #: detector media outra coisa por anos: `confidence_for_matches` só via
    #: similaridade e distância, então "acordo entre vizinhos" só podia ser
    #: sobre as distâncias. O nome escondeu a limitação da forma.
    #:
    #: Opcional porque o caminho antigo de `atlas.atlas_vector_memory` não
    #: tinha o desfecho ao lado do vetor. `None` faz a concordância ser
    #: reportada como ausente, nunca como zero.
...
class SimilarityConfidence(BaseModel):
    ...
    #: Quão UNIFORMEMENTE distantes os vizinhos estão: 1 - amplitude/média.
    #:
    #: ANTES SE CHAMAVA `neighbor_agreement`, E O NOME ERA O DEFEITO. Ele
    #: sugere "os vizinhos concordam", e o detector o usava como portão sob
    #: essa leitura. Ele mede se os K vizinhos estão todos IGUALMENTE longe —
    #: e num top-K ordenado por distância o leque é garantido por construção,
    #: porque o primeiro é o mais próximo que existe e o último é o 25º.
    #:
    #: Medido contra o corpus, o portão em 0,40 deixava passar de 0,0% a
    #: 29,7% das vizinhanças, e ZERO na lente `gols` do Brasileirão e da
    #: Argentina. Pior: penalizava a lente mais forte — `resultado` na Premier
    #: League (+12,7% sobre a taxa base) passava em 9,7%.
    #:
    #: O número continua aqui porque é uma medida real da forma da
    #: vizinhança. Só não é concordância, e agora o nome diz isso.
    distance_uniformity: UnitScore

    #: A FRAÇÃO DOS VIZINHOS QUE TERMINARAM DO MESMO JEITO. Concordância de
    #: verdade, sobre desfecho.
    #:
    #: `None` quando os vizinhos não trazem desfecho — e aí o portão não tem
    #: como julgar e diz isso, em vez de julgar por outra coisa.
    outcome_agreement: float | None = Field(default=None, ge=0.0, le=1.0)
    #: O desfecho da maioria, quando há concordância a reportar.
    modal_outcome: str | None = None
