...
class SimilarityService:
    ...
    def _build_context(
        ...
        return SimilarityContext(
            ...
            # Concordância de desfecho quando os vizinhos a trazem; a
            # uniformidade das distâncias quando não trazem. O caminho antigo
            # de `atlas.atlas_vector_memory` não guardava o desfecho ao lado
            # do vetor, então ali só a segunda existe.
            agreement=(
                confidence.outcome_agreement
                if confidence.outcome_agreement is not None
                else confidence.distance_uniformity
            ),
            coverage=coverage,
