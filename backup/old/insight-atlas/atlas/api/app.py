...
from atlas.api.routes import intake as intake_routes
from atlas.api.routes import query as query_routes
...
# `atlas.similarity` e `atlas.identity` FICAM: alimentam a sonda de
# similaridade da pipeline de trends e o resolvedor canônico do streaming,
# que continuam rodando. As tabelas deles estão vazias porque esses caminhos
# nunca receberam evento — que é diferente de decorativo, e a distinção
# decide se remover quebra algo.
...
from atlas.vector.identidade_ao_vivo import IdentidadeDaPartida, ResolvedorAoVivo
from atlas.vector.service import QueryService
...
def build_app() -> FastAPI:
    ...
    # Traduz o que o evento ao vivo sabe (nomes, instante) para o que o corpus
    # entende (club_id, temporada). Usa o MESMO registro de clubes e a MESMA
    # regra de uid — um segundo entendimento de "que clube é este" é como 352
    # partidas do Arsenal de Sarandí foram parar no Arsenal de Londres.
    resolvedor_ao_vivo = ResolvedorAoVivo(session_factory)
    ...
    trends_pipeline = TrendIntelligencePipeline(
        ...
        similarity_probe=OnlineSimilarityProbe(QueryService(session_factory)),
    )
    ...
    # O REPOSITÓRIO FICA; o watcher que o alimentava a partir do lake saiu.
    # `team_strength_state`, `head_to_head_state` e `team_standings_state`
    # continuam sendo lidos pela consulta ao vivo — foi remover esta linha
    # junto com o watcher que derrubou o serviço em laço de reinício, pela
    # terceira vez nesta reconstrução: atributo referenciado depois que a
    # construção dele foi apagada.
    ...
    container = AppContainer(
        ...
        # `vector_memory`, `ingestion` e `datasets` saíram na limpeza de
        # 2026-08-11. O primeiro era a memória vetorial de duas versões
        # (32 e 37 dimensões, 14 constantes) que `atlas.vector` substituiu;
        # o segundo, o contrato de ingestão do Explorer que `atlas.intake`
        # substituiu; o terceiro, um registro de datasets que nunca recebeu
        # uma linha. As três tabelas foram apagadas junto com o código.
    ...
    # Depois da construcao do container, como o vector_refresher: atribuir
    # antes de `container` existir foi o que derrubou o servico em laco de
    # reinicio duas vezes nesta reconstrucao.
    container.intake_repository = IntakeRepository(session_factory)
    container.query_service = QueryService(session_factory)
    container.vector_builder = VectorBuilder(session_factory)
    ...
    async def run_trends(
        ...
        try:
            ...
            # A IDENTIDADE COMO O CORPUS A ENTENDE, montada aqui porque é
            # aqui que os nomes ainda existem. O resolvedor de identidade os
            # recebe e os descarta ao cunhar o UUID — e sem eles a sonda
            # histórica não tem como achar a partida, que foi a causa de ela
            # passar a casar por estado em jogo contra um corpus de pré-jogo.
            #
            # Falha é um valor, não uma exceção: um provedor mandando um nome
            # novo não pode derrubar a ingestão do evento.
            identidade_do_corpus = None
            if resolvedor_ao_vivo is not None:
                payload_evento = event.get("payload") or {}
                resolvida = await resolvedor_ao_vivo.resolver(
                    competition=str(
                        payload_evento.get("competition_key")
                        or event.get("competition_key")
                        or ""
                    ) or None,
                    home_name=_nome_do_time(payload_evento.get("home_team")),
                    away_name=_nome_do_time(payload_evento.get("away_team")),
                    kickoff=_instante_do_pontape(payload_evento),
                )
                if isinstance(resolvida, IdentidadeDaPartida):
                    identidade_do_corpus = resolvida
                else:
                    logger.debug(
                        "atlas_identidade_ao_vivo_incompleta",
                        extra={
                            "canonical_match_id": str(result.canonical_match_id),
                            "campo": resolvida.campo,
...
def _nome_do_time(valor: object) -> str | None:
    """O nome do time, venha ele como texto ou como objeto aninhado.

    Os provedores discordam da forma: uns mandam `"home_team": "Arsenal"`,
    outros `"home_team": {"name": "Arsenal", "id": 42}`. Aceitar as duas aqui
    evita que a identidade falhe por um detalhe de embalagem.
    """
    if isinstance(valor, str):
        return valor.strip() or None
    if isinstance(valor, dict):
        for chave in ("name", "club_id", "short_name", "display_name"):
            nome = valor.get(chave)
            if isinstance(nome, str) and nome.strip():
                return nome.strip()
    return None


def _instante_do_pontape(payload: dict) -> datetime | None:
    """O pontapé, com fuso. Sem fuso não há como saber o dia UTC — e o dia
    entra na identidade da partida."""
    for chave in ("kickoff_utc", "kickoff", "start_time", "scheduled_at"):
