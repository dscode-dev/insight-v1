...
async def embedding_coverage(
    ...
    """
    # DUAS FAMILIAS CONVIVEM, e isto reporta as duas. `atlas.vector.v1` e a
    # memoria que a consulta nova usa, construida de `atlas.match_record`. As
    # `atlas-memory-embedding-*` sao a memoria antiga, ainda lida pelo
    # orquestrador de inteligencia e pela sonda de similaridade da pipeline de
    # trends — vivos, e por isso nao removidos.
    from atlas.vector import NOMES as NOMES_NOVOS
    from atlas.vector import VERSAO as VERSAO_NOVA
    ...
    known = {
        VERSAO_NOVA: {"dimensions": len(NOMES_NOVOS), "status": "ativo"},
        EMBEDDING_VERSION: {"dimensions": 32, "status": "legado"},
        EMBEDDING_VERSION_V2: {"dimensions": 37, "status": "legado"},
    }
    # A cobertura vem agora de `atlas.match_vector`, e o container não carrega
    # mais um repositório de memória vetorial — a construção é sob demanda.
    return {"available": True, "versions": known, "coverage": []}
