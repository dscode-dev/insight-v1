...
class Trend(BaseModel):
    ...
    #: A identidade da partida COMO O CORPUS A ENTENDE: competição em slug,
    #: temporada, e os dois clubes por `club_id`.
    #:
    #: POR QUE É CAMPO E NÃO UMA CHAVE NO `context`. A sonda a procurava no
    #: dicionário sob quatro grafias possíveis — `competition`,
    #: `competition_key`, `competicao`... — porque o caminho canônico nunca
    #: fixou uma. Procurar por várias grafias é o mesmo que não ter contrato:
    #: quando o produtor escolhe uma quinta, nada quebra e a sonda fica
    #: inerte.
    #:
    #: `None` é legítimo e frequente: watchers que só olham cotações não têm
    #: como saber quem joga. Aí a sonda não consulta o corpus e diz por quê,
    #: em vez de montar uma consulta com o que sobrou.
