...
def atlas_operations(ready: Callable[[], bool], active_jobs: Callable[[], int]) -> OperationsService:
    return OperationsService(
        identity=Identity(
            service_id="atlas",
            service_name="Atlas",
            service_type=operations_pb2.SERVICE_TYPE_LLM,
            version="0.1.0",
            environment=os.getenv("INSIGHT_ENVIRONMENT", ""),
            tags=("llm", "context", "classification", "retrieval"),
        ),
        capabilities=[
            Capability("retrieval", "Retrieves match context and model metadata."),
            Capability("classification", "Classifies contextual signals and trend families."),
            Capability("metrics_generation", "Publishes Atlas runtime metrics."),
        ],
        ready=ready,
