...
def measure_dimensions(
    vectors: list[list[float]], names: tuple[str, ...], *, bias_index: int | None
) -> list[DimensionStat]:
    """Per-position statistics.

    `bias_index` names a component that was fixed at 1.0 before
    normalisation. Dividing by it undoes the normalisation and exposes a
    value that never moved — the trap that made `line_movement` look like
    6.490 distinct values when it is 0.5 in every row.

    Pass None when the layout has no such term. The ratio test is then
    skipped and constancy falls back to counting distinct values, which is
    correct for a standardised space: a feature that never varies is exactly
    0.0 in every row after centring, and stays 0.0 through normalisation.
    Passing index 0 instead of None made dimension 0 a ratio with itself —
    always 1.0, always "constant" — and the ruler reported a dimension that
    varies across 3.784 values as carrying no information.
    """
    if not vectors:
        return []
    width = len(vectors[0])
    columns = [[row[i] for row in vectors] for i in range(width)]
    bias = columns[bias_index] if bias_index is not None else None
    ...
    for index in range(width):
        ...
        # like its own value.
        if bias is None:
            valores = {round(v, 9) for v in column}
            constant = len(valores) == 1
            fixed = next(iter(valores)) if constant else None
        else:
            ratios = {
